...
var _reactAddonsShallowCompare2 = _interopRequireDefault(
...
var Nestable = (function(_Component) {
  ...
  function Nestable(props) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (Nestable.__proto__ || Object.getPrototypeOf(Nestable)).call(this, props)
    );
      ...
      if (itemIds == "NONE") {
        _this.setState({
          collapsedGroups: collapsed
            ? (0, _utils.getAllNonEmptyNodesIds)(items, childrenProp)
            : []
        });
      } else if (itemIds == "ALL") {
        _this.setState({
        ...
        _this.setState({
          collapsedGroups: (0, _utils.getAllNonEmptyNodesIds)(
            items,
            childrenProp
          ).filter(function(id) {
    ...
    _this.onMouseMove = function(e) {
      ...
      var elCopy = document.querySelector(
    ...
    _this.onMouseEnter = function(e, item) {
      ...
      _this.moveItem(
        { dragItem: dragItem, pathFrom: pathFrom, pathTo: pathTo },
        collapseProps
      );
    };
      ...
      var newState = {
        collapsedGroups:
          isCollapsed ^ collapsed
            ? collapsedGroups.filter(function(id) {
                return id != item.id;
              })
    ...
    },
    {
      ...
      value: function tryIncreaseDepth(dragItem) {
        ...
        if (itemIndex > 0 && newDepth <= maxDepth) {
          var prevSibling = this.getItemByPath(
    ...
    },
    {
      ...
      value: function getSplicePath(path) {
        ...
        path.forEach(function(index, i) {
          if (i === lastIndex) {
            currentPath.$splice = [
              [index, numToRemove].concat(_toConsumableArray(itemsToInsert))
            ];
          } else {
            var nextPath = {};
            currentPath[index] = _defineProperty(
              {},
              options.childrenProp,
              nextPath
            );
...
Nestable.propTypes = {
  ...
  group: _propTypes2.default.oneOfType([
